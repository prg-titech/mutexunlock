...
start:
	...
	case *ast.ExprStmt:
		...
		if call, ok := s.X.(*ast.CallExpr); ok && !b.mayReturn(call) {
			...
			b.current = b.newBlock(call.Pos(), "unreachable.call")
		}
	...
	case *ast.ReturnStmt:
		b.add(s)
		b.current = b.newBlock(s.Pos(), "unreachable.return")
	...
	case *ast.IfStmt:
		...
		_else := done
		if s.Else != nil {
			_else = b.newBlock(s.Else.Pos(), "if.else")
		}
		b.add(s.Cond)
		b.ifelse(then, _else)
		b.current = then
...
func (b *builder) branchStmt(s *ast.BranchStmt) {
	...
	if block == nil {
		block = b.newBlock(token.NoPos, "undefined.branch")
	}
	b.jump(block)
	b.current = b.newBlock(token.NoPos, "unreachable.branch")
}
...
func (b *builder) switchStmt(s *ast.SwitchStmt, label *lblock) {
	...
	done := b.newBlock(s.Body.End()+1, "switch.done")
	if label != nil {
		label._break = done
	}
	...
	for i, clause := range s.Body.List {
		body := fallthru
		if body == nil {
			body = b.newBlock(s.Body.Pos(), "switch.body") // first case only
		}

		// Preallocate body block for the next case.
		fallthru = done
		if i+1 < ncases {
			fallthru = b.newBlock(s.Body.Pos(), "switch.body")
		}
		...
		for _, cond := range cc.List {
			nextCond = b.newBlock(cond.Pos(), "switch.next")
...
func (b *builder) typeSwitchStmt(s *ast.TypeSwitchStmt, label *lblock) {
	...
	done := b.newBlock(s.Body.End()+1, "typeswitch.done")
	if label != nil {
		label._break = done
	}
	...
	for _, clause := range s.Body.List {
		...
		body := b.newBlock(s.Body.Pos(), "typeswitch.body")
		var next *Block
		for _, casetype := range cc.List {
			next = b.newBlock(casetype.Pos(), "typeswitch.next")
...
func (b *builder) selectStmt(s *ast.SelectStmt, label *lblock) {
	...
	done := b.newBlock(s.Body.End()+1, "select.done")
	if label != nil {
		label._break = done
	}
	...
	for _, cc := range s.Body.List {
		...
		body := b.newBlock(s.Body.Pos(), "select.body")
		next := b.newBlock(cc.Pos(), "select.next")
		b.ifelse(body, next)
		b.current = body
		b.targets = &targets{
			tail:   b.targets,
...
func (b *builder) forStmt(s *ast.ForStmt, label *lblock) {
	...
	body := b.newBlock(s.Body.Pos(), "for.body")
	done := b.newBlock(s.Body.End()+1, "for.done") // target of 'break'
	loop := body                                   // target of back-edge
	if s.Cond != nil {
		loop = b.newBlock(s.Cond.Pos(), "for.loop")
	}
	cont := loop // target of 'continue'
	if s.Post != nil {
		cont = b.newBlock(cont.Pos, "for.post")
	}
	if label != nil {
		label._break = done
...
func (b *builder) rangeStmt(s *ast.RangeStmt, label *lblock) {
	...
	loop := b.newBlock(s.For, "range.loop") // FIXME
	b.jump(loop)
	b.current = loop

	body := b.newBlock(s.Body.Pos(), "range.body")
	done := b.newBlock(s.Body.End()+1, "range.done")
	b.ifelse(body, done)
	b.current = body
...
func (b *builder) labeledBlock(label *ast.Ident) *lblock {
	...
	if lb == nil {
		lb = &lblock{_goto: b.newBlock(label.Pos(), label.Name)}
		if b.lblocks == nil {
...
func (b *builder) newBlock(pos token.Pos, comment string) *Block {
	g := b.cfg
	block := &Block{
...
func (b *builder) jump(target *Block) {
	...
	target.Preds = append(target.Preds, b.current)
	b.current = nil
}
...
func (b *builder) ifelse(t, f *Block) {
	...
	t.Preds = append(t.Preds, b.current)
	f.Preds = append(f.Preds, b.current)
	b.current = nil
}
